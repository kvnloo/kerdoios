...
| | |
|---|---|
| **This repo** | https://github.com/kvnloo/kerdoios |
...
1. Clone or open https://github.com/kvnloo/kerdoios. `cd` into that tree. `git fetch origin`. `python3 .verified-oss-loop/rollout.py show`. Branch from `origin/$(python3 .verified-oss-loop/rollout.py get worker_base)` unless the issue names another base. Day-pass PRs target `feature_target`. Overnight PRs target `overnight_target`.
...
```bash
gh issue list --repo kvnloo/kerdoios --label claimable --state open
gh pr list --repo kvnloo/kerdoios --state open
```
...
6. Fail, then pass (`skills/tdd/SKILL.md`). Unit command: `python -m pytest`. Mutation: `mutmut run` (write `n/a` if that is what the tree says; do not invent a score).
...
8. Open a PR on https://github.com/kvnloo/kerdoios at `feature_target` (day) or `overnight_target` (overnight) with `.github/PULL_REQUEST_TEMPLATE.md` filled. Bind evidence to `head_revision`. **Do not merge `main` or `dev`.**
