...
class AntiSlopTests(unittest.TestCase):
    ...
    def test_optimize_does_not_restate_free_mode_guard(self) -> None:
        text = (ROOT / "kerdoios/optimize.py").read_text()
    ...
    def test_plan_explain_do_not_advertise_unread_fixture_flag(self) -> None:
        text = (ROOT / "kerdoios/__main__.py").read_text()
    ...
    def test_explain_schema_does_not_claim_last_inventory(self) -> None:
        shim = (ROOT / "__init__.py").read_text()
        pkg = (ROOT / "kerdoios/__init__.py").read_text()
        self.assertNotIn("last inventory", shim.lower())
        self.assertNotIn("last inventory", pkg.lower())
    ...
    def test_capability_score_for_is_not_unread(self) -> None:
        text = (ROOT / "kerdoios/types.py").read_text()
        self.assertNotIn("def score_for(", text)

    def test_one_line_provider_wrappers_are_not_unread(self) -> None:
        fixture = (ROOT / "kerdoios/providers/fixture.py").read_text()
        openrouter = (ROOT / "kerdoios/providers/openrouter.py").read_text()
