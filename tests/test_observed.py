...
from kerdoios.blend import MAX_BLEND_WEIGHT, apply_observed, blend_offer
...
class BlendTests(unittest.TestCase):
    ...
    def test_blend_weight_never_exceeds_cap(self) -> None:
        ...
        stats = aggregate(rows)[(offer.provider, offer.model)]
        blended = blend_offer(offer, stats)
        # completion_rate is 1.0 here so capability should not exceed the
        # original claim scaled by (1-weight) + 1.0*weight == original when
        # completion is perfect; assert the weight itself respects the cap
        # indirectly via a partial-completion case instead.
        self.assertLessEqual(MAX_BLEND_WEIGHT, 0.8)
        _ = blended
