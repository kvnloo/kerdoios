...
**In progress:** inventory every provider/model; keep free as the initial list. OpenRouter is the aggregator. Groq/Cerebras overlay needs vault-resolved keys (Hermes Bitwarden), not `.env`. Vault is a plugin prerequisite: `python3 -m kerdoios doctor` / `hermes plugins doctor` fail closed until `hermes secrets bitwarden setup`.
