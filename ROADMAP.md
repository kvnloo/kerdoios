...
**In progress:** inventory every provider/model; keep free as the initial list. OpenRouter is the aggregator. Groq/Cerebras overlay needs keys. Tokenomics watchdog: burn rate per origin provider (reuse observed JSONL; OpenRouter `:free` catalog GET).
